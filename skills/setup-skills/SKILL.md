...
- **Verifiers** — the commands that decide whether work is good
...
The defaults are the six canonical roles, each label string equal to its name: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `staged`, `wontfix`. On **yes**, write them as-is. Only if the user says no — usually because their tracker already uses other names (e.g. `bug:triage` for `needs-triage`) — collect the overrides so `triage` applies existing labels instead of creating duplicates.
...
- If `CLAUDE.md`/`AGENTS.md` exists, edit it.
...
[the full-gate command]. See `docs/agents/verifiers.md`.
```
